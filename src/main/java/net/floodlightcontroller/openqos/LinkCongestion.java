 ...
 *
 */
public class LinkCongestion extends SwitchResourceBase implements IFloodlightModule {
	...
	public void startUp(FloodlightModuleContext context)
		 ...
		 pollStatsTask.reschedule(POLL_STATS_TASK_INTERVAL, TimeUnit.MILLISECONDS);
	}
	
	/* You create a request and add it to OFStatisticsRequest and 
	 * send is as a query by using .sendQuery with a MessageListener 
	 * so that when the switch responds, you need to process it asynchronously.
	 * So some synchronization difficulty come into play too. 
	 * However all this is done and available though the REST API of floodlight, 
	 * so all that you need to do is query /wm/core/switch/all/port/json 
	 * for all port statistics of each port including the no of bytes received and transfered 
	 * and query /wm/topology/links/json for the information on the topology, 
	 * and it gives you the list of entries with the srcMAC, srcPORT and dstMAC,dstPORT. 
	 * Then you need to query /wm/device/ to get the list of devices ie their MAC IDS and 
	 * which switch they are connected to and at what port. Using these two JSONs 
	 * you can generate the entire topology and get all the links available. 
	 * Then querying /wm/core/switch/all/port/json , gives you the list of MAC IDs 
	 * of the Switches and the ports and the amount /number of packets received and transmitted.
	 * By querying this every now and then you can get the bandwidth of all the given links 
	 * in the entire graph and store it in a Map of a Map of Double.
	 */
